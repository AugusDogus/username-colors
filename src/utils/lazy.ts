...
const isUnconfigurable = (key: PropertyKey) =>
  typeof key === 'string' && unconfigurable.has(key);
...
const lazyHandler: ProxyHandler<any> = {
    ...
    Object.getOwnPropertyNames(Reflect).map((fnName) => {
      return [
        ...
        (target: any, ...args: any[]) => {
          ...
          if (!resolved)
            throw new Error(
              `Trying to Reflect.${fnName} of ${typeof resolved}`
            );
  ...
  ownKeys: (target) => {
    ...
    unconfigurable.forEach(
      (key) => !cacheKeys.includes(key) && cacheKeys.push(key)
    );
    return cacheKeys;
  },
  getOwnPropertyDescriptor: (target, p) => {
    ...
    if (!resolved)
      throw new Error(
        `Trying to getOwnPropertyDescriptor of ${typeof resolved}`
      );
 ...
 */
export function proxyLazy<T, I extends ExemptedEntries>(
  factory: () => T,
  opts: LazyOptions<I> = {}
): T {
  let cache: T;
      ...
      get(_, property) {
        if (property === Symbol.iterator) {
          return function* () {
            ...
            throw new Error(
              'This is not a real iterator, this is likely used incorrectly'
            );
          };
        }
