...
export function getLazyContext<A extends unknown[]>(
  proxy: any
): LazyModuleContext<A> | void {
...
export function createLazyModule<A extends unknown[]>(filter: FilterFn<A>) {
  ...
  const context: LazyModuleContext<A> = {
    ...
    forceLoad() {
      ...
      if (!cache)
        throw new Error(
          `${filter.uniq} is ${typeof cache}! (id ${
            context.moduleId ?? 'unknown'
          })`
        );
      return cache;
    },
  ...
  const proxy = proxyLazy(() => context.forceLoad(), {
    exemptedEntries: {
      ...
      [_patcherDelaySymbol]: (cb: (exports: any) => void) =>
        context.getExports(cb),
    },
  });
